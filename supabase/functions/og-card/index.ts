    ...
    const parts=[
        ...
        excerptLines.length>0
            ?`<rect x="${PX}" y="${exBorderY}" width="3"
                height="${excerptLines.length*25+4}" fill="#ff0000"/>`
            :'',

        // Excerpt lines
        ...excerptLines.map((l,i)=>
            `<text x="${PX+18}" y="${exY+i*25}"
                font-family="${FF}"
                font-size="14" fill="#555555">${x(l)}</text>`),
...
async function generateImage(p) {
    ...
    // Vertical layout — calculate how much space remains for excerpt
    let cy=78; cy+=34;
    const titleY=cy; cy+=titleLines.length*36+14;
    const authorY=cy; cy+=authorText?30:8;
    const starsY=stars?cy+6:-1; cy+=stars?36:0; cy+=6;
    ...
    // Available height: from exY to just above email line (y=600), line height=25px
    const EMAIL_Y    = 600;
    const LINE_H     = 25;
    const maxLines   = Math.max(1, Math.floor((EMAIL_Y - exY) / LINE_H));
    // Feed enough text — ~60 chars per line is safe for mixed CJK/Latin at fontSize 14
    const maxChars   = maxLines * 60;
    const allLines   = wrapText(p.excerpt, 625, 14, false);
    const excerptLines = allLines.slice(0, maxLines);
    // Add ellipsis to last line if there's more text beyond what's shown
    if (allLines.length > maxLines && excerptLines.length > 0) {
        const last = excerptLines[excerptLines.length - 1];
        excerptLines[excerptLines.length - 1] = trunc(last, last.length - 1) + '…';
    }
