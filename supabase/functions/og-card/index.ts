...
function starsSvg(rating, x, y) {
    ...
    return Array.from({length:5}, (_,i) =>
        `<text x="${x+(i*30)}" y="${y}" font-size="24" font-family="Arial,sans-serif" fill="${i<r?'#ff0000':'#dddddd'}">${i<r?'★':'☆'}</text>`
    ).join('');
}
...
<text x="448" y="47" font-size="16" font-family="Arial,sans-serif" fill="#ffffff" text-anchor="middle" font-weight="bold">A</text>
<text x="476" y="47" font-size="15" font-family="Arial,sans-serif" fill="#1a1a1a" font-weight="bold">ACER BOOKS</text>
