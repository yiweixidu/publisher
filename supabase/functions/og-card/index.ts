// @ts-nocheck
// supabase/functions/og-card/index.ts  v5 — TTF font loading
...
async function init() {
    ...
    // 2. Load bundled TTF font (file lives next to index.ts in the function directory)
    try {
        const fontFile = await Deno.readFile(new URL('./NotoSans-Regular.ttf', import.meta.url));
        fontBuffers = [fontFile];
        console.log(`Font loaded from bundle (${fontFile.byteLength} bytes)`);
    } catch(e) {
        console.error('Failed to load bundled font:', e.message);
    }
...
function starsSvg(rating, x, y) {
    ...
    return Array.from({length:5},(_,i)=>
        `<text x="${x+i*30}" y="${y}" font-size="24" font-family="sans-serif" fill="${i<r?'#ff0000':'#dddddd'}">${i<r?'★':'☆'}</text>`
    ).join('');
}
...
<p>Loading&hellip; <a href="${esc(canonicalUrl)}">Click here</a></p>
<script>window.location.replace('${canonicalUrl.replace(/'/g,"\\'")}');</script>
</body></html>`;
        return new Response(html,{status:200,headers:{...CORS,
